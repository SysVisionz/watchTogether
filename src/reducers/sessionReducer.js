...
export default (state=INITIAL_STATE, action) => {
    ...
    switch (type) {
        case 'insert':
            return payload.session ? insertSub(payload.session, state) : null;
        case 'update':
            return payload.props.session ? replaceSub(payload.prop.session, payload.value, state) : null;
        case 'exit':
            return {...INITIAL_STATE, current: payload.};
        case 'exit failed': 
